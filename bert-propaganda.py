...
def write_to_file_flair_corpus(fileout, sentences, tags):
    ...
    for sentence, label in zip(sentences, tags):
        wb.write('__label__' + label + '__label__ ' + sentence + '\n')
    wb.close()
...
if __name__ == '__main__':
    params = {'train_folder': "../data/train",
              ...
              'results': "bert-propaganda.txt",
              'max_seq_length': 210,
              'model': "bert",
              'version_model': "bert-base-cased",
              'model_dir': "propaganda-bert-filter-20epochs",
              'batch_size': 16,
              'epochs': 20,
              'train': True,
              'learning_rate': 0.01,
    ...
    classifier = TextClassifier(document_embeddings, label_dictionary=label_dictionary, loss_weights=weights)
